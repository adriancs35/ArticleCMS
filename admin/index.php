...
<html>
	<head>
		<meta charset="UTF-8" />
		<title>ArticleCMS - <?php echo $lang['admin_panel']; ?></title>
	...
	<body>
		...
		if (!isset($_SESSION['adminuser']) OR empty($_SESSION['adminuser'])){
			?>
			<div id="warpper">
				<img src="../content/images/logo.png" alt="Logo" /><br/>
				<b style="font-size: 35px;"><a class="cms" style="color: orange;">Article</a>CMS</b>
				<div class="auth">
					<?php
					...
					elseif (!empty($_POST['username']) && !empty($_POST['password'])){
						...
						$mysqli -> select_db($database);
							...
							$loginquery = $mysqli -> query("SELECT * FROM accounts WHERE username='".$username."' AND password='".$password."'");
							...
							if ($verify > 0){
								...
								$account_access = $mysqli -> query("SELECT * FROM accounts WHERE id='$id' AND securityLevel='1'");
				...
				<div id="footer">
					<b><a class="cms">ArticleCMS</a></b> © <?php echo date("Y"); ?>
				</div>
			</div>
		<?php
		}
			...
			<div id="cpanel">
				...
				<div id='content'>
					...
					<div class="content">
						...
						<b><a class="cms">Article</a>CMS</b> @ <?php echo date("Y"); ?>
					</div>
				</div>
				<div class="clear"></div>
			</div>
